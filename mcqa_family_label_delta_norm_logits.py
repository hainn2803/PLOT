...
def compute_iia(
    ...
    if var_name == "answer_pointer":
        ...
        num_outside_targets = (
            (target < 0)
            | (target >= pointer_num_labels)
        ).sum().item()

        print(
            "[AP target diagnostic]",
            "num targets outside 0..3=",
            int(num_outside_targets),
        )

        # --------------------------------------------
        # Compare 26-way vs 4-way prediction
        # --------------------------------------------
        pred_26 = scores.argmax(
            dim=-1
        )
        ...
        print(
            "[AP prediction diagnostic]",
            "26-way predictions outside A-D=",
            int(num_pred_outside_4),
            "/",
            int(scores.shape[0]),
        )
