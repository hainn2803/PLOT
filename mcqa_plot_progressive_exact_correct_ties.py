    ...
    for var_id, var_name in enumerate(names):
        ...
        for resolution in resolutions:
            ...
            for top_k in top_k_values:
                ...
                for strength in strength_values:
                    iia, correct = eval_intervention(model, cal_bank, var_name, selected_sites, weights, source_states, bases, float(strength), label_ids, batch_size=batch_size)
                    result = {"var_id": var_id, "var_name": var_name, "stage_A_layers": layer_key, "resolution": int(resolution), "top_k": len(selected_sites), "strength": float(strength), "cal_iia": float(iia), "cal_correct": int(correct), "selected_indices": selected_indices, "selected_sites": selected_sites, "selected_weights": weights, "cache_key": cache_key}
        ...
        print(f"\n[Stage B BEST] var={var_name} correct={best_correct}/{len(cal_bank['base_input_ids'])} ties={len(best)}")
