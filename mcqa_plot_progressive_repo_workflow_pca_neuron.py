...
def site_signature(model, bank, sites, label_ids, mode="neuron", k=None, batch_size=32, strength=1.0, max_fit_states=4096, family_order=FAMILY_ORDER):
    """Build neural-site signatures using either native-neuron or PCA interventions on D_ft."""
    ...
    for site in sites:
        ...
        signatures.append(aggregate_family_signature(diff, bank["pair_source_families"], family_order))
...
def eval_intervention(model, bank, var_name, sites, weights, source_states, bases, strength, label_ids, batch_size=32):
    """Run one native/PCA intervention and return IIA for one target variable."""
    ...
    for var_id, var_name in enumerate(names):
        ...
        for rank, site in enumerate(top_sites, start=1):
            ...
            for strength in strength_values:
                iia = eval_intervention(model, cal_bank, var_name, [site], [1.0], source_states, coarse_bases, float(strength), label_ids, batch_size=batch_size)
...
def run_plot_progressive(model, tokenizer, layers, ft_size=128, cal_size=128, te_size=256, dataset_size=None, dataset_split="train", stage_A_mode="neuron", stage_B_mode="neuron", stage_A_k=None, stage_B_k=None, stage_A_eps=0.001, stage_B_eps=0.001, stage_A_top_layers=6, stage_A_keep_layers=1, stage_A_iia_threshold=0.7, resolutions=(128, 144, 192, 256, 288, 384, 576, 768), top_k_values=(1, 2, 3, 4, 5), strength_values=(1, 2, 4, 8, 16, 32, 64), stage_A_strength_values=None, stage_A_method="uot", stage_B_method="ot", chosen_token_position_id="last_token", device="cuda", seed=0, batch_size=32, max_fit_states=4096):
    ...
    coarse_sig = site_signature(model, ft_bank, coarse_sites, label_ids, mode=stage_A_mode, k=stage_A_k, batch_size=batch_size, max_fit_states=max_fit_states, family_order=FAMILY_ORDER)
    S_coarse = coarse_sig["intervention_diff"]
    T_coarse = stage_A_solver(G_sig, S_coarse, eps=stage_A_eps)
    coarse_bases = coarse_sig["bases"]
    print("[Stage A]", "mode=", stage_A_mode, "dim=", coarse_dim, "shapes=", G_sig.shape, S_coarse.shape, T_coarse.shape)
    ...
    for var_id, var_name in enumerate(names):
        ...
        for resolution in resolutions:
            ...
            if cache_key not in fine_cache:
                ...
                sig = site_signature(model, ft_bank, sites, label_ids, mode=stage_B_mode, k=stage_B_k, batch_size=batch_size, max_fit_states=max_fit_states, family_order=FAMILY_ORDER)
            ...
            for top_k in top_k_values:
                ...
                for strength in strength_values:
                    iia = eval_intervention(model, cal_bank, var_name, selected_sites, weights, source_states, bases, float(strength), label_ids, batch_size=batch_size)
                    result = {"var_id": var_id, "var_name": var_name, "stage_A_layers": layer_key, "resolution": int(resolution), "top_k": len(selected_sites), "strength": float(strength), "cal_iia": float(iia), "selected_indices": selected_indices, "selected_sites": selected_sites, "selected_weights": weights, "cache_key": cache_key}
    ...
    for var_id, best in best_by_var.items():
        ...
        test_iia = eval_intervention(model, te_bank, var_name, best["selected_sites"], best["selected_weights"], source_states, cached["bases"], best["strength"], label_ids, batch_size=batch_size)
        result = dict(best)
    ...
    return {"names": names, "G": G_sig, "S_coarse": S_coarse, "T_coarse": T_coarse, "top_layers_var": top_layers, "top_coarse_by_var": top_coarse, "stage_A_cal_results": stage_A_results, "best_by_var": best_by_var, "stage_B_cal_results": stage_B_results, "test_results": test_results, "fine_cache": fine_cache, "stage_A_mode": stage_A_mode, "stage_B_mode": stage_B_mode}
