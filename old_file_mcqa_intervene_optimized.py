...
def collect_site_activations(
    ...
    if return_output:
        ...
        outputs = torch.softmax(outputs, dim=-1) # normalize output
...
def site_signature(
    ...
    base_states, normalized_base_output = collect_site_activations(
        model=model,
    ...
    num_answers = normalized_base_output.shape[-1]
    ...
    for site_start in range(0, num_sites, site_chunk_size):
        ...
        for batch_start in range(0, N, batch_size):
            ...
            before_b = normalized_base_output[batch_start:batch_end]
