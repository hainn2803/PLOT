...
from mcqa_data_new import (
    build_bank,
    build_target_specific_cal_test_banks,
    letter_token_id,
)
...
def answer_logits_from_model_output(
    model,
    outputs,
    attention_mask,
    ...
    """
    Return raw answer-label logits at the last real token.

    This is used for family_label_delta_norm site signatures.
    No softmax is applied.
    """
    ...
    hidden_at_pos = outputs.last_hidden_state[
        rows,
        logit_pos,
        :,
    ]
...
def answer_probs_from_model_output(
    ...
    logits = answer_logits_from_model_output(
        model=model,
        outputs=outputs,
...
def collect_site_activations(
    ...
    for start in range(0, N, batch_size):
        ...
        try:
            ...
            if return_output:
                if return_logits:
                    batch_output = answer_logits_from_model_output(
                        model=model,
                        outputs=outputs,
                        attention_mask=mask,
                        answer_label_ids=answer_label_ids,
                    ...
                    # Keep signature logits in float32 before
                    # family mean / centering / L2 normalization.
                    batch_output = batch_output.detach().to(
                        dtype=torch.float32,
                        device="cpu",
                    )

                else:
                    batch_output = answer_probs_from_model_output(
                        model=model,
                        outputs=outputs,
                        attention_mask=mask,
                        answer_label_ids=answer_label_ids,
    ...
    for start in range(0, N, batch_size):
        ...
        try:
            ...
            if return_logits:
                ...
                # Preserve logit precision for signature building.
                batch_output = batch_output.detach().to(
                    dtype=torch.float32,
                    device="cpu",
                )
...
def site_signature(
    ...
    For each candidate site:
        ...
        2. Run intervention
        3. Compute per-example answer-label logit delta
        4. Mean within each source family
        5. Center each family block
        6. L2-normalize each family block
        7. Concatenate family blocks
    ...
    for site_idx in range(num_sites):
        ...
        # ----------------------------------------------------
        # Per-example neural effect in answer-label logit space
        #
        # Shape:
        #   [N, num_answers]
        # ----------------------------------------------------
...
def compute_iia(
    ...
    target_labels,
    var_name,
    pointer_num_labels=4,
):
    """
    Variable-specific IIA evaluation.

    answer_pointer:
        Evaluate only over the 4 answer-position symbols A-D.
        Since answer_label_ids are ordered A-Z, these are columns 0:4.

    answer_token:
        Evaluate over the full 26-letter alphabet A-Z.

    Note:
        output_probs are softmax probabilities over A-Z. Restricting AP to
        columns 0:4 before argmax gives the same prediction as taking a
        fresh 4-way softmax over those same four logits, because softmax
        preserves ordering.
    """
    scores = torch.as_tensor(output_probs)

    if scores.ndim != 2:
        raise ValueError(
            "output_probs must have shape [N, num_labels]"
        )
    ...
    if var_name == "answer_pointer":
        if scores.shape[1] < int(pointer_num_labels):
            raise ValueError(
                "Not enough output columns for 4-way "
                "answer_pointer evaluation"
            )

        if bool((target < 0).any()) or bool(
            (target >= int(pointer_num_labels)).any()
        ):
            raise ValueError(
                "answer_pointer targets must be indices 0..3. "
                "With answer_letters=A..Z and base choices A-D, "
                "the current mcqa_data.py should satisfy this."
            )

        eval_scores = scores[
            :,
            :int(pointer_num_labels),
        ]
...
def run_plot_progressive(
    ...
    # 1. Build banks
    # ============================================================
    # Shared fit bank, matching the repo's shared train rows.
    ft_bank = build_bank(
        model=model,
        tokenizer=tokenizer,
    ...
    # Target-specific sensitive holdout banks:
    #   cal_banks["answer_pointer"] -> only AP-changing rows
    #   cal_banks["answer_token"]   -> only AT-changing rows
    # and likewise for te_banks.
    cal_banks, te_banks = build_target_specific_cal_test_banks(
        model=model,
        tokenizer=tokenizer,
        target_variable=target_variable,
        cal_size=cal_size,
    ...
    for var_id in range(len(names)):
        ...
        current_cal_bank = cal_banks[var_name]
        ...
        for site_pos in range(len(top_sites)):
            ...
            cal_source_states = collect_site_activations(
                model=model,
                input_ids=current_cal_bank["source_input_ids"],
                attention_mask=current_cal_bank["source_attention_mask"],
                position_by_id=current_cal_bank["source_position_by_id"],
                sites=[site],
                batch_size=32,
                return_output=False,
                answer_label_ids=answer_label_ids,
            ...
            for strength in stage_A_strength_values:
                cal_output = run_intervention(
                    model=model,
                    input_ids=current_cal_bank["base_input_ids"],
                    attention_mask=current_cal_bank["base_attention_mask"],
                    position_by_id=current_cal_bank["base_position_by_id"],
                    sites=[site],
                    site_weights=[1.0],
                ...
                cal_iia = compute_iia(
                    ...
                    target_labels=current_cal_bank[
                        "counterfactual_label_ids"
                    ][var_name],
                    var_name=var_name,
                )
    ...
    for var_id in range(len(names)):
        ...
        for resolution in resolutions:
            ...
            if cal_cache_key not in stage_B_cal_source_cache:
                cal_source_states = collect_site_activations(
                    model=model,
                    input_ids=current_cal_bank["source_input_ids"],
                    attention_mask=current_cal_bank["source_attention_mask"],
                ...
                stage_B_cal_source_cache[cal_cache_key] = (
                    cal_source_states
                )
    ...
    for var_id in range(len(names)):
        ...
        current_te_bank = te_banks[var_name]
        ...
        te_source_states = collect_site_activations(
            model=model,
            input_ids=current_te_bank["source_input_ids"],
            attention_mask=current_te_bank["source_attention_mask"],
            position_by_id=current_te_bank["source_position_by_id"],
            sites=selected_sites,
            batch_size=32,
            return_output=False,
            answer_label_ids=answer_label_ids,
        ...
        te_output = run_intervention(
            model=model,
            input_ids=current_te_bank["base_input_ids"],
            attention_mask=current_te_bank["base_attention_mask"],
            position_by_id=current_te_bank["base_position_by_id"],
            sites=selected_sites,
            site_weights=selected_weights,
